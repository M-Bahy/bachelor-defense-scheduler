...
function InternalConst() {
  ...
  const loadExaminars = (searchExaminar) => {
    return axios.get(`http://${ipAddr}/allExternals/`).then((res) => {
      ...
      return list.filter((d) =>
        d.label.toLowerCase().includes(searchExaminar.toLowerCase())
      );
    });
  };
  ...
  const loadDates = (searchDate) => {
    return axios.get(`http://${ipAddr}/allExternals/`).then((res) => {
      ...
      return list.filter((d) =>
        d.label.toLowerCase().includes(searchDate.toLowerCase())
      );
    });
  };
  ...
  const go = (event) => {
    event.preventDefault();
    toast("Submitted successfully redirecting ...");
    setTimeout(() => {
      navigate("/table");
    }, 5500);
  };
    ...
    return (
      <>
        <form className={"Constarints-1"}>
          <div className="divTitle">Internal's Constraints</div>
