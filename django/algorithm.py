...
def drawschedule(f):
    ...
    for x in range(len(f[0])):
        print(f[0][x]['Examiner'])
        print(f[0][x]['Supervisor'])
        print(f[0][x]['Room'])
    ...
    for Examiner in solution[1]:
        ...
        for i in range(slots):
            if len(solution[1][Examiner][i]) > 1:
                while(flagc):
                    if(solution[0][c]["Examiner"]==Examiner and solution[0][c]["Time"]==i):
                        solution[0][c]["Color"]="more than 2 per slot for examiner"
                        flagc=False
                    c+=1
    flagc=True
    c=0 
    ...
    for Supervisor in solution[2]:
        ...
        for i in range(slots):
            if solution[2][Supervisor][i] > 1:
                while(flagc):
                    if(solution[0][c]["Supervisor"]==Supervisor and solution[0][c]["Time"]==i):
                        solution[0][c]["Color"]="more than 2 per slot for supervisor"
                        flagc=False
                    c+=1
    flagc=True
    c=0  
    ...
    for Examiner in solution[1]:
        ...
        for i in range(len(l)):
            ...
            if len(solution[1][Examiner][l[i]]) >= 1:
                while(flagc):
                    if(solution[0][c]["Examiner"]==Examiner and solution[0][c]["Time"]==l[i]):
                        solution[0][c]["Color"]="slot of examiner in external constraint"
                        flagc=False
                    c+=1
    flagc=True
    c=0 
